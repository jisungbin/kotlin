    ...
    private fun VariableInitializationInfoData.processUnionNode(
        ...
        fun CFGNode<*>.reportErrorsOnInitializationsInInputs(
            ...
            requireWithAttachment(
                condition = visited.add(this),
                message = {
                    """Node has already been visited and could result in infinite recursion.

Declaration: ${this.containingDeclaration()?.symbol?.fqName}
Variable: ${symbol.fqName}

Node: ${this.render()}
Visited:
 - ${visited.joinToString(separator = "\n - ") { it.render() }}

Path: ${path.label}
Graph Tree:
${this.owner.renderGraph()}"""
                },
            ) {
                withFirSymbolEntry("variable", symbol)
                containingDeclaration()?.let {
                    withFirEntry("declaration", it)
                    withFirSymbolIdEntry("declarationId", it.symbol)
                }
            }
...
private fun ControlFlowGraph.renderGraph(): String {
    fun recurse(graph: ControlFlowGraph, prefix: String, builder: StringBuilder) {
        builder.append(prefix).append("- ").appendLine(graph.name)
        for (previous in graph.enterNode.previousNodes.map { it.owner }) {
            recurse(previous, prefix = "$prefix    ", builder)
        }
    }

    return buildString { recurse(graph = this@renderGraph, prefix = "", builder = this@buildString) }
}
